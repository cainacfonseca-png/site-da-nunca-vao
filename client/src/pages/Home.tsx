...
export default function Home() {
  ...
  const generatedStoreProducts = generatedProducts.filter((product) => product.section === "store");
  const generatedFindsProducts = generatedProducts.filter((product) => product.section === "finds");
  const products = (generatedProducts.length
    ? [
        ...(generatedStoreProducts.length ? generatedStoreProducts : databaseProducts.filter((product) => product.section === "store")),
        ...(generatedFindsProducts.length ? generatedFindsProducts : databaseProducts.filter((product) => product.section === "finds")),
      ]
    : databaseProducts.length ? databaseProducts : initialProducts) as Product[];
  ...
  return <main className="site-shell"><header className="site-header"><a href="#top" className="brand" aria-label="Nunca Vão, início"><img className="brand-logo" src="/brand/logo-nunca-vao.png" alt="Nunca Vão" /><span className="brand-name">NUNCA VÃO</span></a><a className="instagram-link" href="https://instagram.com/nunca.vao" target="_blank" rel="noreferrer" aria-label="Instagram Nunca Vão"><Instagram size={19} /></a></header><section className="hero" id="top"><div className="hero-orbit orbit-one" /><div className="hero-orbit orbit-two" /><div className="hero-cross" /><div className="hero-content"><span className="hero-tag">Para quem torce</span><h1>Coisas que a gente encontra.<br /><em>Paixões que ficam.</em></h1><p>A gente não faz produto do time. Faz produto de quem torce.</p><div className="hero-actions"><a href="#store" className="primary-button">Ver a loja <ArrowUpRight size={17} /></a><a href="https://www.umapenca.com/nuncavao" target="_blank" rel="noreferrer" className="secondary-button">Loja na UmaPenca <ArrowUpRight size={16} /></a><a href="#finds" className="text-button">Ver achadinhos</a></div></div><div className="hero-stamp"><span>Desde</span><strong>1898</strong><span>até sempre</span></div></section><div className="content-wrap"><div className="catalog-toolbar"><div><span className="eyebrow">Escolha seu próximo achado</span><h2>Vitrine da bio</h2></div><label className="search-field"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar produto" aria-label="Buscar produto" /></label></div><Section section="store" products={storeProducts} /><Section section="finds" products={findsProducts} /></div><footer className="site-footer"><div className="footer-brand"><img className="footer-logo" src="/brand/logo-nunca-vao.png" alt="Nunca Vão" /><strong>NUNCA VÃO</strong></div><p>Produtos para quem torce. Uma curadoria feita com paixão e memória.</p><a href="https://www.umapenca.com/nuncavao" target="_blank" rel="noreferrer">Comprar na UmaPenca <ArrowUpRight size={15} /></a></footer></main>;
}
